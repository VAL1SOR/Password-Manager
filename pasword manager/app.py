...
open("passwords.exe", "a").close()
os.system("attrib +h passwords.exe")
root = Tk()
...
def save_password():
    open("passwords.exe", "a").write(w.get() + " " + w2.get() + "\n")
    for _ in open("passwords.exe", "r"):
        if (_ not in save):
            save.append(_)
...
def get_random_string():
    ...
    if ((t.get() == 0) and (v.get() == 0) and (c.get() == 0) and (r.get() == 0)):
        pass
    else:
